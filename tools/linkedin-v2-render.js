// Render the improved Design 1 v2 to PNG on Desktop.
...
(async () => {
  ...
  // Full-size retina output
  const page = await context.newPage();
  const fileUrl = 'file:///' + path.resolve(__dirname, 'linkedin-1-hero-v2.html').replace(/\\/g, '/');
  await page.goto(fileUrl, { waitUntil: 'networkidle' });
  await page.waitForTimeout(2500);
  const outPath = path.join(os.homedir(), 'Desktop', 'ankommer-linkedin-1-hero-v2.png');
  ...
  const thumbPath = path.join(os.homedir(), 'Desktop', 'qc-linkedin-1-hero-v2.png');
