...
const ThemeManager = {
  ...
  set: (theme) => {
    ...
    // Keep the phone drawer's proxy theme button in sync (the nav toggle is
    // display:none on phones, so screen-reader users rely on this one for state).
    const railBtn = document.querySelector('.rail-theme-btn');
    if (railBtn) railBtn.setAttribute('aria-pressed', theme === 'dark' ? 'true' : 'false');
